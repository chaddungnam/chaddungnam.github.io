...
- Quirky Ball의 최신 붉은 쿼키 슈팅 캡처와 Project K의 기존 세로 MP4·포스터 사용
...
- 붉은 쿼키는 공식 `quirky_ball_logo.png`에서 분리한 제품 이미지를 사용하며 임의 캐릭터로 다시 그리지 않는다.
...
- 칠곡할매 권안자체는 공식 배포처와 웹 사용 허용 범위를 확인한 뒤 짧은 사람 냄새 강조 문구가 실제로 남을 때만 추가한다. 현재 승인 시안처럼 해당 문구가 없으면 폰트도 추가하지 않는다.
...
4. 기존 `assets/brand-site.js`가 로컬 JSON을 읽어 네 언어 홈의 카드 3개를 갱신한다.
...
- 공개 피드에 영상이 3개보다 적으면 있는 카드만 표시하되 빈 가짜 카드는 만들지 않는다.
...
- 기존 18초 `assets/media/quirky-ball-gameplay.mp4`는 드롭·머지만 보여주므로, 현재 게임의 붉은 회전 슈팅과 조준 슈팅을 포함한 검증 캡처로 같은 경로를 갱신한다.
...
- 공통 메뉴·언어·피드 동작은 기존 `assets/brand-site.js`를 확장한다.
- 기존 로고, Montserrat, Do Hyeon, 게임 포스터와 Project K MP4를 재사용하고 Quirky Ball MP4만 현재 기믹 캡처로 갱신한다.
...
| 실제 규칙 기반 Quirky 슈팅 | Canvas 2D와 공식 붉은 쿼키 이미지 | 발사 주기·쌍발·1회 반사·축소 검사, 프레임 변화 확인 |
| 최신 영상 3개 자동 갱신 | YouTube 워크플로, 동기화 스크립트, JSON, `brand-site.js` | 정상·잘못된 피드 실행 검사와 카드 DOM 검사 |
| 같은 크기의 영상 카드 | 공통 카드 CSS | 데스크톱 카드 높이 비교 |
| 상단이 돌출된 두 아이폰 목업의 실제 영상 | 최신 Quirky 캡처·기존 Project K MP4·홈 마크업 | 36~60px 돌출, 재생 속성, 포스터, 화면 밖 정지, 브라우저 화면 |
