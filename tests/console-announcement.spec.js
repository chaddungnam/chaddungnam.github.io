...
test("operator can publish a notice from operations", async ({ page }) => {
  ...
  expect((await page.evaluate(() => window.__noticePayload)).content).toEqual({version: 1, blocks: [{type: "paragraph", text: "1.1.1 공지 테스트입니다.", size: "normal", align: "left", bold: false, italic: false, underline: false}]});
...
test("operator can edit an existing notice including start and end dates", async ({ page }) => {
  ...
  await page.locator("#announcementDocument [data-paragraph]").fill("1.1.1 공지 본문과 날짜를 수정합니다.");
  ...
  await expect.poll(() => page.evaluate(() => window.__noticePayload)).toMatchObject({
    ...
    reason: "게시 시작일 수정",
    startsAt: await page.evaluate(() => new Date("2026-09-02T09:00").toISOString()),
    endsAt: await page.evaluate(() => new Date("2026-09-16T09:00").toISOString()),
  });
...
test("notice edit clearly explains that a required reason is missing", async ({ page }) => {
  ...
  await page.locator('#noticeTask').evaluate(node => { node.open = true; });
  await page.locator("[data-edit-notice='7']").click();
  await page.locator("#announcementDocument [data-paragraph]").fill("수정했지만 사유는 비어 있습니다.");
  await page.locator("#announcementForm button[type=submit]").click();

  await expect(page.locator("#announcementMessage")).toHaveText("수정·발행 사유를 입력해 주세요. 아직 서버에는 반영되지 않았습니다.");
  await expect(page.locator("#announcementDocument [data-paragraph]")).toHaveText("수정했지만 사유는 비어 있습니다.");
});
...
test("notice translation failure keeps the draft and shows a useful message", async ({ page }) => {
  ...
  await page.evaluate(() => { window.ConsoleApp.confirmChange = async () => true; });
  await page.locator("[data-edit-notice='7']").click();
  await page.locator("#announcementDocument [data-paragraph]").fill("번역 실패에도 보존할 공지");
  ...
  await expect(page.locator("#announcementDocument [data-paragraph]")).toHaveText("번역 실패에도 보존할 공지");
