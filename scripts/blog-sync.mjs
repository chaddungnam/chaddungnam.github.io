...
const LOCALES = {
  kr: { lang: "ko", label: "한국어", back: "모든 포스트", heading: "제작 기록", hero: ["House Duck's Blog,", "만드는 과정을 포스트로 기록합니다."], heroNote: "게임과 소프트웨어를 만들며 겪은 실험, 실패, 수정과 출시 판단을 기록합니다.", home: "메인 페이지 보러가기", games: "제작 중인 게임", release: "출시 준비 중", development: "개발 중", description: "게임과 앱을 만들며 겪은 시행착오와 출시 과정을 기록하는 House Duck 개발 블로그입니다.", skip: "본문으로 건너뛰기", theme: "색상 테마 전환", noteTitle: "공식 블로그", note: "같은 글을", original: "House Duck Blog에서 읽기", auto: false },
  en: { lang: "en", label: "English", back: "All posts", heading: "Build notes", hero: ["House Duck's Blog,", "the making process, one post at a time."], heroNote: "Experiments, mistakes, revisions, and release decisions from building games and software.", home: "Back to the main page", games: "Games in progress", release: "Preparing for release", development: "In development", description: "House Duck build notes about making games and apps, including real decisions, mistakes, and release work.", skip: "Skip to content", theme: "Switch color theme", noteTitle: "Automatic translation", note: "This page was automatically translated from Korean and may contain unnatural wording.", original: "Read the Korean original", auto: true },
  de: { lang: "de", label: "Deutsch", back: "Alle Beiträge", heading: "Entwicklungsnotizen", hero: ["House Duck's Blog,", "der Entstehungsprozess, Beitrag für Beitrag."], heroNote: "Experimente, Fehler, Korrekturen und Veröffentlichungsentscheidungen aus der Entwicklung.", home: "Zur Hauptseite", games: "Spiele in Entwicklung", release: "Veröffentlichung in Vorbereitung", development: "In Entwicklung", description: "Entwicklungsnotizen von House Duck über Spiele, Apps, Entscheidungen, Fehler und Veröffentlichungen.", skip: "Zum Inhalt springen", theme: "Farbschema wechseln", noteTitle: "Automatische Übersetzung", note: "Diese Seite wurde automatisch aus dem Koreanischen übersetzt und kann unnatürliche Formulierungen enthalten.", original: "Koreanisches Original lesen", auto: true },
  ja: { lang: "ja", label: "日本語", back: "すべての記事", heading: "開発記録", hero: ["House Duck's Blog,", "作る過程を一つずつ記事に残します。"], heroNote: "ゲームやソフトウェア制作での実験、失敗、修正、リリース判断を記録します。", home: "メインページへ", games: "制作中のゲーム", release: "リリース準備中", development: "開発中", description: "ゲームやアプリを作る中での判断、失敗、改善、リリース過程を記録するHouse Duckの開発ブログです。", skip: "本文へ移動", theme: "カラーテーマを切り替える", noteTitle: "自動翻訳", note: "このページは韓国語から自動翻訳されているため、不自然な表現が含まれる場合があります。", original: "韓国語の原文を読む", auto: true },
};
...
function renderBlogFooter(locale) {
  const labels = {
    kr: ["이용약관", "개인정보처리방침", "고객지원", "비즈니스 문의"],
    en: ["Terms", "Privacy", "Support", "Business inquiries"],
    de: ["Nutzungsbedingungen", "Datenschutz", "Support", "Geschäftsanfragen"],
    ja: ["利用規約", "プライバシー", "サポート", "ビジネスのお問い合わせ"],
  }[locale];
  const support = locale === "kr" ? "/support/" : "/support/#english";
  return `<footer class="mirror-footer site-footer"><div class="footer-inner"><div><div class="footer-brand-images"><img class="brand-duck-image" src="/assets/house-duck-logo.png" width="512" height="512" alt=""><img class="footer-wordmark-image" src="/assets/house-duck-wordmark.png" width="1694" height="394" alt="House Duck"></div><small>© <span data-current-year>2026</span> House Duck.</small></div><nav class="footer-links" aria-label="Legal and support"><a href="/terms/${LOCALES[locale].lang}.html">${labels[0]}</a><a href="/privacy/${LOCALES[locale].lang}.html">${labels[1]}</a><a href="/impressum/${LOCALES[locale].lang}.html">Impressum</a><a href="${support}">${labels[2]}</a><a href="mailto:business@houseduck.in">${labels[3]} · business@houseduck.in</a></nav></div></footer>`;
}
...
function renderBlogHero(locale) {
  const copy = LOCALES[locale];
  const siteLanguage = locale === "kr" ? "ko" : locale;
  const title = copy.hero.join(" ");
  return `<section class="manifesto-hero mirror-manifesto" aria-labelledby="blog-hero-title"><div class="manifesto-bubble"><span class="manifesto-mark" aria-hidden="true">HD</span><div class="manifesto-dialogue" data-typewriter><h1 id="blog-hero-title" aria-label="${escapeHtml(title)}">${copy.hero.map((line) => `<span data-type-line>${escapeHtml(line)}</span>`).join("")}</h1><p class="manifesto-note"><span data-type-line>${escapeHtml(copy.heroNote)}</span></p></div><a class="manifesto-action" href="/${siteLanguage === "ko" ? "" : `index_${siteLanguage}.html?lang=${siteLanguage}`}">${escapeHtml(copy.home)} →</a><span class="quirky-sticker" aria-hidden="true"><img src="/assets/media/quirky-character.svg" alt=""></span></div><div class="device-stage" aria-label="${escapeHtml(copy.games)}"><a class="game-device game-device-quirky" href="/quirky-ball/?lang=${siteLanguage}"><span class="game-device-label"><strong>Quirky Ball</strong><small>${escapeHtml(copy.release)}</small></span><span class="iphone-shell"><span class="phone-side-button phone-side-button-top"></span><span class="phone-side-button phone-side-button-bottom"></span><span class="phone-camera" aria-hidden="true"></span><video data-game-preview autoplay muted loop playsinline preload="metadata" poster="/assets/media/quirky-ball-gameplay-poster.jpg" aria-label="Quirky Ball"><source src="/assets/media/quirky-ball-gameplay.mp4" type="video/mp4"></video><span class="phone-home-indicator" aria-hidden="true"></span></span></a><a class="game-device game-device-k" href="/project-k/?lang=${siteLanguage}"><span class="game-device-label"><strong>Project K</strong><small>${escapeHtml(copy.development)}</small></span><span class="iphone-shell"><span class="phone-side-button phone-side-button-top"></span><span class="phone-side-button phone-side-button-bottom"></span><span class="phone-camera" aria-hidden="true"></span><video data-game-preview autoplay muted loop playsinline preload="metadata" poster="/assets/media/project-k-highlight-poster.jpg" aria-label="Project K"><source src="/assets/media/project-k-highlight.mp4" type="video/mp4"></video><span class="phone-home-indicator" aria-hidden="true"></span></span></a></div></section>`;
}

function renderIndexPage(posts, locale, archivedCards = []) {
  const copy = LOCALES[locale];
  const currentCards = posts.map(({ post, content }, index) => `<article><a href="/blog/${locale}/${encodeURIComponent(post.slug)}/">${post.image ? `<img src="${escapeHtml(post.image)}" alt="${escapeHtml(content.title)}" loading="${index === 0 ? "eager" : "lazy"}"${index === 0 ? ' fetchpriority="high"' : ""}>` : ""}<div><time datetime="${post.publishedAt.toISOString()}">${escapeHtml(localizedDate(post.publishedAt, locale))}</time><h2 data-preview-type>${escapeHtml(content.title)}</h2><p data-preview-type>${escapeHtml(content.summary)}</p></div></a></article>`);
