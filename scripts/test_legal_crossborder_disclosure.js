...
for (const locale of ['ko', 'en', 'de', 'ja']) {
  ...
  for (const token of ['eu-west-1', 'Supabase Pte. Ltd.', 'privacy@supabase.io', 'SCC', 'FCM', 'APNs', '2026-09-09']) assert.ok(privacy.includes(token), `${locale}: missing ${token}`);
  for (let n=1;n<=13;n++) assert.equal((privacy.match(new RegExp(`id="section-${n}"`, 'g'))||[]).length,1,`${locale}: privacy anchor ${n}`);
  assert.ok(terms.includes('2026-09-09'));
